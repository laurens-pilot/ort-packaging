...
3. Confirm representative-device correctness and performance evidence, including Android WebGPU on the supported physical-device matrix. A packaging-only release may cite the evidence from its immutable source release because it reuses the same verified machine code.
4. Dispatch `release.yml` from `main` with the computed tag and `prerelease=false`. Use `scope=all` for a fresh native build. For a packaging-only correction, use `scope=repackage` and provide the immutable 35-asset `source_tag`. To replace only Linux binaries, use `scope=linux` with a source tag; the workflow rebuilds and tests Linux x64/ARM64 while reusing the verified non-Linux binaries. Both source-release modes verify every source checksum and record per-asset provenance.
