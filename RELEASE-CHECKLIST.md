...
4. Dispatch `release.yml` from `main` with the computed tag and `prerelease=false`. Use `scope=all` for a fresh native build. For a packaging-only correction, use `scope=repackage` and provide the immutable 35-asset `source_tag`; the workflow verifies every source checksum, records per-asset provenance, and reruns iOS packaged-runtime inference.
